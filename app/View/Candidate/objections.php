...
<div class="d-flex">
<button class="btn bg-primary text-white text-1xl w-15" style="margin-left:30vh;" onclick="openPopupPassObjectionId()" data-objection_id="<?php echo $value->ObjectionID ?>">Respond</button>
<button class="btn bg-primary text-white text-1xl w-15" style="margin-left:25vh;" onclick="openPopupUsingId('popup1')" data-objection_id="<?php echo $value->ObjectionID ?>" data-respond="<?php echo $value->Respond ?>">View Respond</button>
    ...
    <?php $id= $value->ObjectionID;?>  <!-- The ID of the row. This is a unique number. -->
        <div class="popup mx-1 my-1 px-1 py-1 min-w-40 min-h-50">
            <div class="title">My respond</div>
                <form action="/ezvote/Candidates/respondObjection/<?= $value->CandidateID ?>/<?= $value->ElectionID ?>" method="POST" class="d-flex flex-column my-1 px-1 align-items-flex-start">
                    <div class="d-flex flex-column my-1 w-100">
                        <label for="Description" class="mr-1 text-left text-md"><b>Description</b></label>
                        <textarea id="Description" class="border-1" name="Description" style="height:150px; width:100%; resize:none;"></textarea>
                    </div>
                    <div><input type="text" style="display:none" name="ObjectionID" value=""> </input></div>
                    <div class="d-flex justify-content-between my-1 w-100">
                    <div><button class="btn btn-danger px-1" onclick="closePopup()">Cancel</div>
                    <div><button class="btn btn-success px-1" type="submit">Submit</div>
            </div>
                </form>
        </div>
</div>

<div class="dialog-box-outer" id="popup1">
    <?php $id= $value->ObjectionID;?>  <!-- The ID of the row. This is a unique number. -->
    <div class="popup mx-1 my-1 px-1 py-1 min-w-40 min-h-50">
        <div class="m-3 border-2 border-dark p-3 border-radius-5" style="box-shadow: 5px 10px #888888">
            <h2 class="bg-success">Respond</h2>
            <p class="text-xl m-2 objrespond"><?= $value->Respond ?></p>
        <div>
            <input type="text" style="display:none" name="ObjectionID" value="<?php echo $id; ?>"> </input>
        </div>
    </div>
            <button class="btn btn-danger" onclick="closePopupUsingId('popup1')">Cancel
</div>
